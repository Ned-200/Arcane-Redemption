...
public class DisintegrateUP : MonoBehaviour
{
    ...
    // Optional compatibility with your old calls
    ...
    private IEnumerator DisintegrateRoutine(bool dissolveOut)
    {
        ...
        if (runtimeMats == null || runtimeMats.Length == 0)
        {
            Debug.LogError($"[DisintegrateUP] No runtime materials available on {name}");
            running = false;
            activeRoutine = null;
            yield break;
        }
    ...
    private void PrepareRuntimeMaterials()
    {
        ...
        for (int i = 0; i < originalMats.Length; i++)
        {
            ...
            if (debugMaterials)
            {
                Debug.Log($"[DisintegrateUP] Slot {i}: source '{sourceMat.name}' -> runtime '{disMat.name}'");
            }
        }
